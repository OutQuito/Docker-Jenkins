...
Після видалення всіх данних з Docker, та зміни версії nginx для CentOS
помилка нарешті була виправленна, а також контейнер працює штатно що дозволеле продовжети навчання.
# =============== E R R O R =============== 
...
Ви помітите, що Docker Compose руйнує вашу мережу (оскільки її можна легко відтворити), але не видаляє ваш том. Коли він з’являється знову, він відтворює мережу і не турбується про гучність. Розумні інструменти — це чудово!

За замовчуванням docker-compose down видаляє все, крім томів. Якщо вам потрібно також видалити томи, спробуйте: 

    docker-compose -p jenkins down -v

# ВИСНОВКИ

Ми дізналися, що Compose може спростити наше керування командами для запуску, зупинки та створення програми з кількома зображеннями — і все це за низьку ціну ще одного файлу конфігурації. Перевага цього файлу полягає в тому, що він самодокументований у визначенні зв’язків між контейнерами, які ми запускаємо, їхніми мережами та томами.

Compose — це чудовий інструмент, який чітко висловлюється. Я б додав до його потенційного списку недоліків те, що вам потрібно завжди вказувати узгоджену назву проекту. Натомість Compose може керувати більшістю базових операцій, включаючи очищення.

Чи будете ви використовувати Compose, залежатиме як від того, наскільки вам подобаються його самовпевнені підходи, так і від того, наскільки складними стають ваші програми Docker. Мені особисто подобається самодокументований характер файлу docker-compose.yml і спрощення структури команд. Ви помітите, що я все ще надаю make-файл зі спрощеним набором команд, оскільки мені також подобається система, яка зменшує мої шанси забути шаблонний параметр, як-от -p або -d. Це цілком особистий вибір.

На цьому мої основні підручники готові! Тепер у вас є ваш власний Дженкінс у коробці з NGINX, який можна перенести на будь-який докер-хост. Наступні публікації будуть значно розширенішими. 